...
import {isNumber, fakeUser} from './testHelpers.js';
...
test('Get user by id', async t => {
  ...
  t.truthy(req.body, 'Can\'t get one user!')

  t.is(req.body.name, users[1].name)
})
...
test.serial('Change a users language', async t => {
  let newUser = fakeUser();
  let id;
  let req = await request(app)
    .post('/api/users')
...
test.serial('add to a users favorite forums', async t => {
  let newUser = fakeUser();
  let id;
  ...
  t.is(res.status, 200, 'Error creating new user');
  ...
  t.is(res.status, 200, 'Error adding to favorites');

  // CHECK IF USER HAS NEW FORUM
  res = await request(app)
    .get('/api/users/' + id)

  t.is(res.status, 200, 'Error retrieving user');
  t.is(true, Array.isArray(res.body.favorites))

  t.true(res.body.favorites.indexOf('foo') > 0)
})

test.serial('remove from a users favorite forums', async t => {
  let newUser = fakeUser();
  let id;
  ...
  t.is(res.status, 200, 'Error making new user');
  t.truthy(res.body.id);
  id = res.body.id;
  ...
  t.is(res.status, 200, 'Should get 200 from adding forum');

  // Remove from user forums
  res = await request(app)
    .delete('/api/users/forums/' + id + '?favorite=foo')

  t.is(res.status, 200, 'Should get 200 from removing forum');

  // Check that forum has been deleted
  res = await request(app)
    .get('/api/users/' + id);

  t.is(res.status, 200, 'Should get 200 from retrieving user');
...
test.serial('Use queries for get all users', async t => {
  // age
  let res = await request(app)
...
test('uses queries to get admins', async t => {
  let res = await request(app)
    .get('/api/users?type=admin')
    .expect(200)
  
  res.body.forEach(user => t.true(user.type === 'admin'))
})

test.serial('Update one user', async t => {
  let user = fakeUser();

  let res = await request(app)
    .post('/api/users')

  t.is(res.status, 200, 'Should send 200 when making a new user');
  t.truthy(res.body.id);
  t.true(isNumber(res.body.id));
  user.id = res.body.id;
  ...
  t.is(res.status, 200, 'Error retrieving modified user');
  t.truthy(res.body);
